...
UnaryOpNode::UnaryOpNode(ExprPtr operand, UnaryOp op) : Operand(operand), Op(op) {}
...
InitValNode::InitValNode(ExprPtr expr) : Expr(expr), IsList(false) {}

InitValNode::InitValNode(std::vector<InitValNode*>&& list) :
    Expr(nullptr), List(std::move(list)), IsList(true) {}
...
VarDefNode::VarDefNode(std::string name, std::vector<ExprPtr>&& dims,
                       InitValPtr init) :
    Name(std::move(name)), Dims(std::move(dims)), Init(init) {}
...
VarDeclNode::VarDeclNode(BaseType type, std::vector<VarDefPtr>&& defs,
                         bool isConst) :
    Type(type), Defs(std::move(defs)), IsConst(isConst) {}
...
FuncParamNode::FuncParamNode(BaseType type, std::string name) :
    Type(type), Name(std::move(name)) , IsArray(false) {}

FuncParamNode::FuncParamNode(BaseType type, std::string name,
                             std::vector<ExprPtr>&& dims) :
    Type(type), Name(std::move(name)), Dims(std::move(dims)), IsArray(true) {}

void FuncParamNode::accept(Visitor& visitor) const { visitor.visit(*this); }

BlockNode::BlockNode(std::vector<StmtPtr>&& items) : Items(std::move(items)) {}
...
FuncDefNode::FuncDefNode(BaseType returnType, std::string name,
    ...
    ReturnType(returnType), Name(std::move(name)), Params(std::move(params)), Body(body) {}

void FuncDefNode::accept(Visitor& visitor) const { visitor.visit(*this); }

AssignNode::AssignNode(LValPtr dest, ExprPtr src) : Dest(dest), Src(src) {}

void AssignNode::accept(Visitor& visitor) const { visitor.visit(*this); }

ExprStmtNode::ExprStmtNode(ExprPtr expr) : Expr(expr) {}
...
WhileNode::WhileNode(ExprPtr cond, StmtPtr body): Cond(cond), Body(body) {}
...
ReturnNode::ReturnNode(ExprPtr expr) : Expr(expr) {}

void ReturnNode::accept(Visitor& visitor) const { visitor.visit(*this); }

CompUnitNode::CompUnitNode(std::vector<NodePtr>&& items) : Items(std::move(items)) {}
